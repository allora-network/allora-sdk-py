...
import requests
import logging
from typing import Awaitable, Callable, Generic, Optional, AsyncIterator, TypeVar
...
from allora_sdk.worker.forecaster import (
    Forecaster,
    TForecasterRunFn,
    TForecasterRunFnResult,
)
from allora_sdk.worker.inferer import (
    Inferer,
    SanityCheckConfig,
    TInfererRunFn,
    TInfererRunFnResult,
...
SubmissionWindowOpenEventType = TypeVar(
    "SubmissionWindowOpenEventType", bound=TSubmissionWindowOpenEventType
)
WorkerFnReturnType = TypeVar("WorkerFnReturnType")
SequenceProvider = Callable[[AlloraRPCClient, str, int], Awaitable[int]]
SequenceResetFn = Callable[[str], Awaitable[None]]
...
class AlloraWorker(Generic[SubmissionWindowOpenEventType, WorkerFnReturnType]):
    ...
    def inferer(
        ...
        account_sequence_provider: SequenceProvider | None = None,
        account_sequence_reset: SequenceResetFn | None = None,
        debug: bool = False,
    ):
        """
        ...
        return AlloraWorker[EventWorkerSubmissionWindowOpened, TInfererRunFnResult](
            ...
            account_sequence_provider=account_sequence_provider,
            account_sequence_reset=account_sequence_reset,
            debug=debug,
        )
    ...
    def __init__(
        ...
        self._account_sequence_provider = account_sequence_provider
        self._account_sequence_reset = account_sequence_reset
    ...
    async def _log_balance(self):
        ...
        resp = await self.client.bank.query.balance(
            QueryBalanceRequest(address=self.address, denom="uallo")
        )
        ...
        logger.info(
            f"   Worker wallet: {self.address}  ||  Balance: {balance_formatted}"
        )
        return
            ...
            try:
                ...
                for _ in range(MAX_BALANCE_POLLS_PER_FAUCET_REQUEST):
                    ...
                    if balance >= MIN_ALLO:
                        return
                logger.warning(
                    "    Faucet request succeeded but balance did not update in time, retrying..."
                )
            except requests.HTTPError as err:
                if err.response.status_code == 429:
                    logger.error(
                        f"    Too many faucet requests. Try sending ALLO to your worker's wallet manually from another wallet, or visit https://faucet.testnet.allora.network"
                    )
                    self.stop()
                    sys.exit(-1)
            ...
            def signal_handler(signum, frame):
                ...
                if signum == signal.SIGINT:
                    if not sigint_received:
                        ...
                        logger.info(
                            "Received SIGINT, initiating graceful shutdown (Ctrl-C again to force exit)"
                        )
    ...
    async def run(
    ...
    ) -> AsyncIterator[WorkerResult[WorkerFnReturnType] | Exception]:
        """
        ...
        logger.debug(
            f"Starting Allora {self.use_case.name()} for topic {self.topic_id}"
        )
        ...
        try:
            ...
            if did_register:
                logger.info(
                    f"✅ Registered {self.use_case.name()} {self.address} for topic {self.topic_id}"
                )
    ...
    async def _run_with_context(
        self, ctx: Context
    ) -> AsyncIterator[WorkerResult | Exception]:
        await self._ensure_initialized()
    ...
    async def _polling_worker(self, ctx: Context):
        ...
        while not ctx.is_cancelled():
            try:
                ...
                logger.error(
                    f"The wallet {self.address} is not whitelisted on topic {self.topic_id}.  Contact the topic creator."
                )
                self.stop()
                break
    ...
    async def _subscribe_websocket_events(self):
        ...
        await self.client.events.subscribe_new_block_events_typed(
            EventWorkerSubmissionWindowClosed,
            [EventAttributeCondition("topic_id", "=", f'"{str(self.topic_id)}"')],
            lambda evt, height: logger.info(
                f"✨ Worker submission window closed (topic={evt.topic_id} nonce={evt.nonce_block_height} height={height})"
            ),
        )
        await self.client.events.subscribe_new_block_events_typed(
            EventReputerSubmissionWindowOpened,
            [EventAttributeCondition("topic_id", "=", f'"{str(self.topic_id)}"')],
            lambda evt, height: logger.info(
                f"🚀 Reputer submission window opened (topic={evt.topic_id} nonce={evt.nonce_block_height} height={height})"
            ),
        )
        await self.client.events.subscribe_new_block_events_typed(
            EventReputerSubmissionWindowClosed,
            [EventAttributeCondition("topic_id", "=", f'"{str(self.topic_id)}"')],
            lambda evt, height: logger.info(
                f"✨ Reputer submission window closed (topic={evt.topic_id} nonce={evt.nonce_block_height} height={height})"
            ),
        )

        # Subscribe to rewards events for autostaking if configured on the use case
        if (
            isinstance(self.use_case, SupportsAutoStake)
            and self.use_case.autostake is not None
        ):
            await self.client.events.subscribe_new_block_events_typed(
                EventRewardsSettled,
                [EventAttributeCondition("topic_id", "=", f'"{str(self.topic_id)}"')],
    ...
    async def _handle_submission_window_opened_event(
        self, event: SubmissionWindowOpenEventType, height: int
    ):
        await self._ensure_initialized()
        ...
        logger.info(
            f"🚀 {role_name} submission window opened (topic={self.topic_id} nonce={event.nonce_block_height} height={height})"
        )
    ...
    async def _maybe_submit_impl(self, ctx: Context, nonce: Optional[int] = None):
        ...
        async def submit(nonce: int, account_seq: int):
            ...
            try:
                ...
                if isinstance(result, AlreadySubmittedError):
                    logger.info(
                        f"⚠️ Already submitted for this epoch: topic_id={self.topic_id} nonce={nonce} code={result.code}"
                    )
                    ...
                    logger.error(
                        f"⚠️ Transaction timed out: topic_id={self.topic_id} nonce={nonce}"
                    )

                elif isinstance(result, AccountSequenceMismatchError):
                    logger.warning(
                    ...
                    logger.info(
                        f"✅ Successfully submitted: topic={self.topic_id} nonce={nonce}"
                    )
        ...
        if self._account_sequence_provider is not None:
            base_sequence = await self._account_sequence_provider(
                self.client, self.address, len(new_nonces)
            )
        else:
            account_seq = await self.client.auth.query.account_info(
                QueryAccountInfoRequest(address=self.address)
            )
        ...
        for i, nonce in enumerate(new_nonces):
            ...
            logger.info(
                f"👉 Found new nonce {nonce} for topic {self.topic_id}, submitting... account_seq={next_sequence}"
            )
    ...
    @staticmethod
    def _is_sequence_mismatch_error(err: TxError) -> bool:
        text = f"{err.message or ''}".lower()
        return (
            "account sequence mismatch" in text or "incorrect account sequence" in text
        )

    async def _reset_sequence_allocator(self) -> None:
        if self._account_sequence_reset is None:
            return
        try:
            await self._account_sequence_reset(self.address)
            logger.warning(f"Reset shared sequence cache for address {self.address}")
        except Exception as err:
            logger.warning(f"Failed to reset shared sequence cache: {err}")
