...
class UseCase(Protocol[WindowOpenedEvent, UseCaseReturnType]):
    ...
    def requires_sequential_nonces(self) -> bool:
        """True when a nonce may only be submitted once every older nonce of
        the topic is closed. The worker then submits the oldest open nonce
        alone and waits for it to close before moving to the next one."""
        ...
