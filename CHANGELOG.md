...
- Gas headroom is configured by the `AlloraRPCClient`/`TxManager` `gas_adjustment` setting (default `1.2`; also settable via `GAS_ADJUSTMENT`). It multiplies the simulated estimate exactly once; retry escalation is applied separately.
