...
Submits predictions to Allora Network topics with your models. The worker handles wallet creation, blockchain transactions, and automatic retries so that you can focus on model engineering.
...
async def main():
    worker = AlloraWorker(predict_fn=my_model)
...
import allora_sdk.protos.emissions.v9 as emissions

# Initialize client
client = AlloraRPCClient.testnet(mnemonic="your mnemonic...")
...
- **Multi-chain**: Testnet and mainnet support come with batteries included, but there is maximal configurability
...
client = AlloraAPIClient(chain_id=ChainID.TESTNET)
